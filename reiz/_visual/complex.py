...
class Background(Visual):
    ...
    def __init__(self, color="white"):
        self.color = tuple([int(c * 255) for c in get_color(color)])

    def adapt(self, window):
        img = pyglet.image.SolidColorImagePattern(color=self.color)
        img = img.create_image(window.width, window.height)
        self.visual = pyglet.sprite.Sprite(img=img, x=0, y=0, usage="static")
...
class Bar(Visual):
    ...
    def __init__(
        self, height: float = 0.5, width: float = 0.25, color="white"
    ):
        self.height = height
        self.width = width
        self.color = get_color(color)
...
class Circle(Visual):
    ...
    def adapt(self, window):
        ...
        y = (y0 * self.pos[1]) + y0
        self.visual = _Circle(
            x=x, y=y, z=0, width=width, color=self.color, stroke=self.stroke
        )

    def __repr__(self):
        return (
            f"Circle(zoom={self.zoom}, color={self.color}, "
            + f"position={self.pos})"
        )
...
class Cylinder(Visual):
    ...
    def adapt(self, canvas):
        ...
        self.visual = _Polygon(
            v=vertices, z=0, color=self.color, stroke=0, rotation=self.angle
        )
...
class Image(Visual):
    ...
    def adapt(self, window):
        base_scale = min(
            window.width / self.img.width, window.height / self.img.height
        )
        ...
        self.visual = pyglet.sprite.Sprite(
            img=self.img, x=x, y=y, usage="static"
        )
